...
class Tank():
    ...
    def update(self, now):
        ...
        if not self.is_dead:
            ...
            #----------Here goes the player logic----------#
                ...
                if self.player_input_cooldown <= 0:
                    ...
                    try:
                        ...
                        self.acc, self.gun_target, self.shoot_order = self.player_input(self.game.screen_width, self.game.screen_height, self.rect.center, self.vel, self.life, self.gun_dir, self.gun_cooldown, self.name, self.team, self.sonar_reading)
            ...
            if magnitude(self.acc) > self.max_acc:
                self.acc = mult_vector(normalize(self.acc), self.max_acc)
                
            # Substract friction, add acceleration
            friction = 0.99
            self.vel = mult_vector(self.vel, friction)
            self.vel = add_vector(self.vel, self.acc)
            # Cap at max speed
            ...
            self.gun_dir = normalize(add_vector(self.gun_dir, mult_vector(sub_vector(self.gun_target, self.gun_dir), self.turret_speed)))
            ...
            if self.gun_cooldown > 0:
                ...
                self.game.bullets.append(Bullet(self.game, add_vector(self.rect.center, mult_vector(self.gun_dir, 20)), vel=mult_vector(normalize(self.gun_dir), self.shoot_speed), damage=self.gun_damage))
    ...
    def sonar(self):
        ...
        for obj in self.game.batteries + self.game.walls + self.game.tanks:
            if obj is not self:
                dist = magnitude(sub_vector(obj.rect.center, self.rect.center))
            ...
            if coll is not self:
                if self.rect.colliderect(coll):
                    ...
                    if intersection == self.rect:
                        ...
                        push = mult_vector(normalize(sub_vector(self.pos, coll.rect.center)), 2)
                    else:
                        push = mult_vector(normalize(sub_vector(self.pos, intersection)), 2)
                        
                    self.pos = add_vector(push, self.pos)
    ...
    def draw(self):
        ...
        gun_end = add_vector(gun_center, mult_vector(normalize(self.gun_dir), self.gun_size))
        pygame.draw.line(self.game.screen, GRAY, gun_center, gun_end, 5)
        
        # Life
        pygame.draw.line(self.game.screen, RED, add_vector(self.pos, (0,-10)),add_vector(self.pos, (self.size,-10)), 5)
        pygame.draw.line(self.game.screen, GREEN, add_vector(self.pos, (0,-10)),add_vector(self.pos, (self.size*(self.life/100),-10)), 5)
        
        # Shot
        if self.gun_cooldown > self.gun_max_cooldown-80:
            if self.gun_cooldown > self.gun_max_cooldown-20:
                shot_ini = gun_end
                shot_end = add_vector(gun_center, mult_vector(normalize(self.gun_dir), self.gun_size*3))
                shot_size = 20
                ...
                shot_ini = gun_end
                shot_end = add_vector(gun_center, mult_vector(normalize(self.gun_dir), self.gun_size*2))
                shot_size = 5
                pygame.draw.line(self.game.screen, YELLOW, shot_ini,shot_end, shot_size)
        ...
        if self.game.debug:
            pygame.draw.line(self.game.screen, GREEN, self.rect.center, add_vector(self.rect.center, mult_vector(self.acc, 10000)), 1)
